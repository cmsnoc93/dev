...
while(len(s)>0):
    ...
    while boo:
        try:
            ...
            print(best_match)  
            ssh= ConnectHandler(device_type=best_match,host=now,username="rit",password="CMSnoc$1234")
...
while boo:
        try:
            ...
            ssh=ConnectHandler(device_type=best_match,host=dst,username="rit",password="CMSnoc$1234")
            boo=False
        except:
            boo=True
            print(" Connection error, trying again ")
...
for nme in setofnames:
    ...
    while boo:
        ...
        try:
            ...
            try:
                ...
                best_match = guesser.autodetect()
                print(best_match)
                ssh=ConnectHandler(device_type=best_match,host=dictofobj[nme].sship,username="rit",password="CMSnoc$1234")
        ...
        if ff==1:
            ...
            boo=True
        elif not(('soft_ver' in retkey) and ('version' in retkey) and ('uptime' in retkey) and ('hardware' in retkey) and len(ret[0]['hardware'])>0 and ('reload_reason' in retkey)):
            boo=True
            print("9-0 keys not proper in return from show version ")
        else:
            boo=False
    ...
    while boo:
        ...
        try:
            ...
            boo=True
            ff=1
        print("9 Return from sh pro cpu")
        print(ret)
        if ff==1:
            boo=True
        elif not(isinstance(ret,list)):
...
#------------------------------------------Neeraj-----------------------------------------------------------------------------------------------
    ...
    while boo:
        ...
        if not ret:
            ...
            print("9-4 Returned value on show proc mem is not proper- Processor, trying again")
            boo=True
        elif len(ret.split())<6:
